...
直播时长记录逻辑成立的前提条件：不存在任何一场跨月且同时跨越超过一天的直播（例如，8月31日-9月2日）。

1. 下播时保存直播时长统计文件：

若开播时间不足30分钟，不进行任何记录。

下播时记录下播时间戳，读取meta.json的live_time为开播时间戳，并进行以下处理：

- 本场直播未跨月：
    - 用下播时间戳减开播时间戳，并加到livetime.json原有的livetime中，作为本月的总直播时长；
    - 以"yyyymmdd hh:mm:ss - yyyymmdd hh:mm:ss"的格式向livetime.json的exacttime中追加记录；
    - 若开播时间对应的日期不在livetime.json的exactday中，则追加写入；
    - 更新livetime.json的liveday为新的exactday的长度。

这意味着对于任何的跨天直播，都只有前一天会被计入开播天数。

- 本场直播跨月（跨月时livetime.json会被更新，更新格式在下一部分归档部分）：
    - 用下播时间戳减当月1日0:00的时间戳，并加到livetime.json原有的livetime中，作为本月的总直播时长；
    - 以"yyyymmdd hh:mm:ss - yyyymmdd hh:mm:ss"的格式向livetime.json的exacttime中追加记录，这里的开播时间并非当月1日0:00，而是从meta.json的live_time获取的真实开播时间戳；
    - 不对开播日期（即exactday和liveday）信息作更新。

2. 每月归档：

每个月1日的0:00，会对直播时长统计文件livetime.json作归档处理，归档至history_files/livetime中。具体为：

- 若此时正在直播：把开播时间至当月1日0:00的时长追加到旧文件，按照上面的逻辑更新旧文件，然后将旧文件归档。若前一个月的直播时长不足30分钟，则不追加任何信息；
- 若此时未直播：直接归档旧文件。

归档后，创建新文件livetime.json，格式为：
...
若跨月时正在直播，则下播后会按照这个json文件进行处理。新一个月的livetime.json中，对于跨月直播，是否需要记录的逻辑为本场直播的总时长是否大于30分钟。

这意味着，如果跨月直播，前一个月与后一个月的直播时长会被分别计入两个月内，但开播日期只会记录在前一天内。

3. 查询本月开播时长（需权限）：

具有查询权限的用户输入“查时长”、“查开播时长”或“查直播时长”这几个指令时，程序会在livetime.json中查询livetime键值。

- 若正在直播：在已查询到的livetime键值基础上，添加当前时间戳与开播时间的差值。开播时间从meta.json获取，若本场直播跨月则将开播时间修正为当月1日0:00.
- 若未直播：不添加差值，直接计算。

4. 查询本次直播时长（需权限）：

具有查询权限的用户输入“查本次时长”、“查本次直播时长”、“查本场时长”、“查本场开播时长”这几个指令时，函数返回添加当前时间戳与开播时间的差值。当LIVE_STATUS不为1时，函数会直接退出。

5. 查询本月开播天数（需权限）：

具有查询权限的用户输入“查开播天数”或“查直播天数”指令时，程序会在livetime.json中查询liveday键值。处理逻辑如下：

- 若未直播，则返回liveday；
- 若在开播状态，且开播的日期已经在exactday中，则返回liveday；
- 若在开播状态，且目前已经跨月，则返回liveday；
- 其他情况，返回liveday+1，这主要是来处理非跨月且当天第一次开播，直播时长信息还没有写入livetime.json的情况。
...
注：
1. 更新日志与readme的更新时间不一定完全相同（因为写技术文档时间比较长，如果忙的话可能会过些天才写）;
2. 更新日志仅记录与代码**逻辑改动**相关的内容，不记录techdoc文档的更新以及一些代码小错误的修正。

**2026.9.18**

PUSH_STATUS现在可热修改为2，该状态下完全禁止推送姬推送。
...
1. 推送姬下播时，额外推送直播间封面信息；
2. 下播邮件中添加了直播间封面的附件；
3. 新增查询本次直播直播时长的代码。

**2026.9.11**

更新main_v2.py为main_v3.py：由于bilibili_api的send_gift接口失效，将send_gift与danmu接口更换为blivedm.

**2026.8.31**

更新直播时长与直播天数统计程序的统计与查询逻辑，以及其跨月时的处理逻辑。

**2026.8.27**

1. 更新推送姬若有大于1条待推送动态时的延迟3s推送机制；
2. 对于用户进入直播间的监听程序，现在共用INTERACT_WORD_V2与ENTRY_EFFECT两个接口来监听，并设置异步锁防止二次执行。

**2026.8.26**

将监听用户进入直播间的接口从INTERACT_WORD_V2更新为ENTRY_EFFECT.
...
推送姬推送动态时，若有大于1条动态待推送，每条动态之间会延迟3秒钟（原来为同时推送）。

**2026.8.21**

1. 明确FastAPI跳板机制与生命进程的基本原理；
2. 修改PUSH_STATUS逻辑为是否@全体成员；
3. runtime_state.json现在共有已推送总次数与已推送开播下播次数两个维度（原来只有第二个）；
4. 更新推送姬@全体成员的处理逻辑，以及@次数已满时不会@全体成员的处理逻辑（v2026.08.21）。

**2026.8.20**

更新推送姬@全体成员的处理逻辑（v2026.08.20，仅通过已推送开播下播次数决定是否@全体成员）。
...
2. 新增runtime_state.json，用于记录已推送开播下播次数，从而在推送次数较多时拒绝推送。
...
通过并行任务，同时发送舰长群与粉丝群的推送消息（原来为粉丝群相比于舰长群延迟5s发送）。
...
1. 增加每30s轮询开播状态的逻辑；
2. 下播时若内存中保留的开播时间为0（也就是没有识别到开播），则推送姬不推送下播消息。
...
新增SuperChat日志记录，文件保存与superchat.jsonl.

**2026.7.7**

所有盲盒的json数据已增加cost_detail和profit_detail两个key，内含详细的各类盲盒成本与收益信息。

这意味着特定种类盲盒查询不再依赖常量文件提前写明的盲盒可产出内容，但仍保留判断输入文字是否为可查询盲盒的逻辑。
...
1. 下播时，推送姬会向admin邮箱发送开播与下播时间消息，便于直播回放视频的存储；
2. 除SEND_DANMU接口触发的彩蛋以外，其余接口触发彩蛋均以支持热修改（因为弹幕事件触发过于频繁）。
...
3. 修复程序因pm2 stop, pm2 restart或因程序崩溃等原因停止时，保存所有内存信息至json文件的逻辑错误；
4. 增加热删除礼物信息的代码；
5. 修复热更新与热删除时，不能同步修改meta.json信息的问题；
6. 热更新与热删除现已支持增删盲盒信息，同时会对礼物姬“你看又⭕️”的层级与阈值进行修正（热删除时较为保守，按照4000电池逐级回退）。
...
修复程序重启时audience.json数据被异常抹除的问题。其原因为global interact_cache后重新对interact_cache进行赋值，导致产生了幽灵变量。

**2026.5.18**

1. 增加礼物热更新端口；
2. 增加程序被手动停止时，强制将内存信息保存到各个json文件的逻辑；
3. 修复audience.json在程序初始运行时读取异常的问题。

**2026.5.16**
1. 将SESSDATA, BILI_JCT与BUVID3获取的方法集成到get_data.py里，以避免手动复制Windows端数据后ip变化导致bilibili_api数据变化，从而导致账号失效的问题；
2. 增加盲盒姬查询特定种类盲盒的代码；
3. 增加盲盒姬查询特定月份盲盒的代码。
...
机器人排队回复队列中，每条回复的间隔时间修改为2.5-3.5s.

**2026.5.2**

限制欢迎姬欢迎各位早崎鸭的最大次数为1次。
...
所有机器人回复均被置入回复队列排队回复，每条回复的间隔时间为4-5s.

**2026.4.27**

礼物姬现在会对单价电池数高于特定阈值的礼物进行感谢。
