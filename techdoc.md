...
common_notice_danmaku接口提供的字典部分信息：
...
通过asynccontextmanager可以给FastAPI注册生命进程lifespan，在其scheduler中添加带参的cron任务，可实现在特定时间周期性执行指定任务。
...
（由于腾讯的风控比较严，所以NapCat以及其他开源的或者闭源的，总之是非官方的框架都不是很稳定，但也没办法）

（我一个cs的同学跟我说，想做爬虫最容易的平台就是b站，因为风控比较少见hh）
...
每天6:45，transfer.py会先将部分直播数据转换为excel和png，然后transfer.sh会把添加了日期名称的json原始数据文件以及excel和png文件统一转存至history_files文件夹中，从而实现数据存储。

如果下播时间晚于6:45，则需手动再次执行transfer.sh脚本程序，覆盖crontab自动存储的数据文件。

如果下播时间晚于7:59（且开播时间在7:59前），则需将两份不同的文件合并。具体原因可以看下一小部分。

（6:45这个时间是根据云崎早历史最晚下播时间来确定的，如果以后某一次比6:45还要晚那就继续推迟）

### 2.2.4 数据文件的删除

每天7:59，为解决上文提到的7:59还未下播导致本来应该是一个json文件但被重置操作导致切分为了两份的情况，可以先执行transfer.sh，再执行reset.sh. reset.sh会重置服务器数据。具体为：删除files路径下的所有json和jsonl文件，并重启整个机器人程序。这样，json_handle.py会先在files创建初始json文件，之后内存读取这些初始json文件，所有的数据就处于了归零的状态。

为什么不设置成云崎早下播就整理并重置数据，而是一定要设置一个固定时间呢？因为云崎早可能会二次开播，会导致二次下播时第一次下播的数据被直接覆盖。

（为什么设置了7:59这样一个毫无规律的时间呢？是为了防止云崎早设置8点整的定时动态。程序重启需要2-3秒钟的时间，在这2-3秒内程序不会运行。如果设置成8点整，万一云崎早设置了8点定时发动态，由于8点这一瞬间程序没有运行，推送姬就不会监测到有新动态发送，也不会向群里推送消息了）
...
但是，盲盒姬和礼物姬查询没有做这一层限制，意味着盲盒姬和礼物姬可以随时被呼叫。但正如前文提到的，数据会在每天7:59重置，因此只有在7:59前才能查询到数据。
...
constants.py中设置了可供查询的盲盒名称及映射关系，当通过正则提取得到的盲盒名称不在映射列表中时，盲盒姬不会回答用户。
...
push_times表示当天已经推送过的消息数，push_live_times表示当天已经推送过的开播与下播消息次数（动态推送不计入），其会在每天0:00时自动归零，由于是json文件，因此不受每天7:59程序重启的影响。
...
2. 每天7:59之后，即使盲盒姬与礼物姬仍可以查询数据，但由于数据源已经归档，因此查询不到数据；
