...
部分在readme中未提及到，且在2026.8.15前的细节调整：

- 2026.8.12：
  - 修复推送姬在多群推送时，因HTTP长连接复用导致的超时卡顿问题；
  - 修改了推送姬推送消息的日志文本内容，现在若推送报错，日志中可直观显示出错误类型。
- 2026.7.17：增加每30s轮询开播状态的逻辑。
- 2026.7.9：新增SuperChat统计网页，html文件记录在web/superchat/index.html中，该html文件调用superchat.json数据。
- 2026.7.7：特定种类盲盒查询不再依赖常量文件提前写明的盲盒可产出内容，但仍保留判断输入文字是否为可查询盲盒的逻辑。
- 2026.6.3：对于readme同一天提到的问题，日志记录在send_gift.txt中。
- 2026.6.1：
  - box.json生成时，对每个用户增加info的key信息，内含盲盒的原始名称与数量；
  - audience.json生成时，增加total_audience的key信息；
  - 修复程序因pm2 stop, pm2 restart或因程序崩溃等原因停止时，保存所有内存信息至json文件的逻辑错误。
- 2026.5.31：删除GUARD_BUY的大航海统计接口。
- 2026.5.18：增加程序被手动停止时，强制将内存信息保存到各个json文件的逻辑。
- 2026.5.16：将SESSDATA, BILI_JCT与BUVID3获取的方法集成到get_data.py里，以避免手动复制Windows端数据后ip变化导致bilibili_api数据变化，从而导致账号失效的问题。
- 2026.4.26：新增电池计数器网页，html文件记录在web/gift/index.html中，该html文件调用gift.json数据。
