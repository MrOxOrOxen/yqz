...
若跨月时正在直播，则下播后会按照这个json文件进行处理。新一个月的livetime.json中，对于跨月直播是否需要记录的逻辑为本场直播的总时长是否大于30分钟。
...
这意味着，如果跨月直播，两个月的直播时长会被分别计入两个月内。

4. 查询本月开播天数（需权限）：

具有查询权限的用户输入“查开播天数”或“查直播天数”指令时，程序会在livetime.json中查询liveday键值。若开播日期与当前日期在同一个月，且当前日期不在exactday记录中，则将查询到的键值加1；否则，直接返回键值。
