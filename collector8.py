...
async def on_gift(event):
    ...
    # 盲盒（开之后）
    g_id = data_dict.get('data').get('gift_id')
    g_name = data_dict.get('data').get('gift_name')
    g_num = data_dict.get('data').get('combo_num')
    if data_dict['data']['blind_gift'] is not None:
        g_value = data_dict.get('data').get('blind_gift').get('gift_tip_price') / 1000
    else:
        g_value = data_dict.get('data').get('combo_total_coin') / g_num / 1000
    print(f"INFO: 收到来自 [{real_uname}]({real_uid}) 的 {g_name} x{g_num} ({g_value*10}电池) (COMBOx{g_num})")
    handle_logic(
        uid=real_uid,
        user_name=real_uname,
        bg_id=bg_id,
